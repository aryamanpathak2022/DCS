...
for cfg = 1:size(configs, 1)
    ...
    for blk = 1:nBlocks
        ...
        for k = 1:length(SNRdB)
            ...
            mmse = (inv(transpose(H)*H+(No/SNR(k))*eye(t)))*transpose(H);
            DetectedSymbols = mmse * RxSymbols; % MMSE
...
ylabel('BER');
legend('ZF','MMSE');
title('BER vs SNR for QPSK with Zero-Forcing Receiver and MMSE reciever');